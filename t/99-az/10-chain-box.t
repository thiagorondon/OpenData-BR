...
ok($uc);
my $rv = OpenData::AZ::Box->new(
        process_item => sub { shift; return reverse shift }
);
ok($rv);
my $chain = OpenData::AZ::ChainBox->new( chain => [ ( $uc, $rv ) ] );
ok($chain);
...
ok($rp5);
my $cc = OpenData::AZ::Box->new(
    process_item => sub { shift; return length(shift) } );
ok($cc);
my $chain2 = OpenData::AZ::ChainBox->new( chain => [ $rp5, $cc ] );
ok($chain2);
